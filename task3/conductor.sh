...
run()
{
    ...
    #using unshare creating new namespaces and using chroot setting the root directory and the shell will run in it. inside shell
    #mounting sysfs,procfs,giving permission to root directory and running the given program
    unshare --uts --pid --mount --ipc --net --fork --kill-child  chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c "
    mount -t sysfs none /sys &&
    mount -t proc none /proc &&
    chmod 755 / && exec $INIT_CMD_ARGS" 
}
...
exec()
{
    ...
    #using nsenter to join the namespaces of the init process of the container, setting it's root directory and working directory
    #then executing the given command in it.
    nsenter --target "$CONTAINER_INIT_PID" --uts --pid --net --mount --ipc --wd --root /bin/bash -c "exec $EXEC_CMD_ARGS"
}
...
addnetwork()
{
    ...
    sudo ip link add "$INSIDE_PEER" type veth peer name "$OUTSIDE_PEER"
    ...
    #enabling the outside interface from host
    sudo ip link set "$OUTSIDE_PEER" up

    local CONTAINER_INIT_PID=$(pgrep -P $PID | head -1)

    #passing the inside interface to the given namespace
    ip link set "$INSIDE_PEER" netns "$NAME"
    
    #enabling the inside interface and container's loopback interface from container.
    nsenter --target "$CONTAINER_INIT_PID" --uts --pid --net --mount --ipc --wd --root /bin/bash -c "ip link set $INSIDE_PEER up
    ip link set lo up"
