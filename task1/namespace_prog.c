...
int main() {
   ...
   // ------------------ WRITE CODE HERE ------------------
    child_pid = clone(child_function, child_stack + CHILD_STACK_SIZE, CLONE_NEWPID | CLONE_NEWUTS, (void *)pipefd);
    if(child_pid == -1)
    {
    ...
    */
    ...
    // Used pid_fdopen to get the fd for child process 
    int second_child_fd = pidfd_open(child_pid,0);
    if(setns(second_child_fd,CLONE_NEWPID) == -1)
    {
        perror("setns error");
        exit(1);
    }
    ...
    if (fork() == 0) {
        ...
        */
        ...
        // ------------------ WRITE CODE HERE ------------------
        int setns_var = setns(second_child_fd, CLONE_NEWUTS);
        if(setns_var == -1)
        {
            perror("error in setns 2");
            exit(1);
        }
